    ...
    <script language="c#" runat="server">
		...
		public int GetChildrens(Sitecore.Data.Items.Item item,int level=0)
        {
            ...
            if (item != null)
            {
				...
				if (items.Count>100) //disable to get a full list
                {
					level++;
					childrens +=items.Count;
					
                    Response.Write(string.Format("{2} Item [{0}] - Children Count {1} <BR>", item.Paths.FullPath, items.Count, 
					new String('-', level) 
					));
        ...
        public void GetItemCount()
        {
            ...
            using (new SecurityDisabler())
            {
               ...
               Database.GetDatabase("master"));
				...
				Response.Write("Max Childrens count :"+max.ToString()+", Item ["+maxItemPath+"]<BR>");
